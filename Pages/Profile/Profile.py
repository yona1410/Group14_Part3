...
def index(user_id):
    ...
    for obj in bookings:
        # Split the start_date into parts
        start_date_parts = str(obj['start_date']).split()[0].split('-')
        # Construct the formatted start_date string
        obj['start_date'] = f"{start_date_parts[2]}.{start_date_parts[1]}.{start_date_parts[0]}"
        # Split the end_date into parts
        end_date_parts = str(obj['end_date']).split()[0].split('-')
        # Construct the formatted end_date string
        obj['end_date'] = f"{end_date_parts[2]}.{end_date_parts[1]}.{end_date_parts[0]}"

    return render_template('Profile.html',user=user, bookings=bookings)


@Profile.route('/cancel_now/<booking_id>', methods=['DELETE'])
