...
def search_results(place, kosher, animal, elevator, parking, guests, startdate, enddate):
    ...
    not_booked_apartment_ids = [apartment_id for apartment_id in relevant_apartment_ids if apartment_id not in booked_apartment_ids]
    ...
    formatted_startdate = startdate.strftime("%B %d, %Y")
    formatted_enddate = enddate.strftime("%B %d, %Y")

    return render_template('SearchResults.html', query_params=query_params,apartments=apartments,startdate=formatted_startdate, enddate=formatted_enddate)
...
def book_now(apartment_id, start_date, end_date):
    if request.method == 'POST':
        # Retrieve form data
        # Convert formatted_startdate and formatted_enddate back to datetime objects
        startdate_obj = datetime.strptime(start_date, "%B %d, %Y")
        enddate_obj = datetime.strptime(end_date, "%B %d, %Y")

        # Format the datetime objects as strings in the "%Y-%m-%d 00:00:00" format
        startdate_formatted = startdate_obj.strftime("%Y-%m-%d 00:00:00")
        enddate_formatted = enddate_obj.strftime("%Y-%m-%d 00:00:00")
        ...
        new_booking = {
            ...
            'start_date': startdate_formatted,
            'end_date': enddate_formatted,
